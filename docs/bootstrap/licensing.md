...
Bootstrap records the managed license mode and exact content hash in the tracked ownership sidecar as well as local state. The mutable sidecar is a reviewable ownership claim and can preserve fail-closed removal protection, but it cannot independently authorize an overwrite or deletion. When the local state carries no license record, the sidecar's validated `license` entry serves as the prior legal classification only if the on-disk `LICENSE` byte-matches its declared `contentSha256`; such plans carry an explicit `classificationSource: ownership-sidecar` audit line, so a clean clone of an already governed projection can be planned and re-applied without inventing transition evidence. Every case that is not a byte-matching sidecar-governed projection — a missing sidecar license entry, a hash mismatch, changed rendered bytes, or a mode change — remains a fail-closed legal hard stop requiring exact adoption/transition evidence in the manifest; changing managed notices also requires a trusted local baseline or explicit migration. Existing root files named `LICENSE` or `LICENCE` with suffixes or extensions (for example `LICENSE.md` and `LICENSE-MIT`), `COPYING` variants such as `COPYING.LESSER`, `UNLICENSE` variants, and equivalent legal files must be reconciled into the canonical `LICENSE` before Bootstrap can manage licensing.
