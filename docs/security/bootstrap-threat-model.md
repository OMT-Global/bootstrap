...
Bootstrap is a local TypeScript control-plane CLI plus GitHub Actions workflow set. For provenance, the highest risks are publishing sensitive input through a public manifest, accepting forged reviewer or subject lineage, trusting an unsigned or improperly verified manifest, and later allowing private-bundle or sink failures to fail open. The current local public contract reduces disclosure risk with strict schema allowlists and typed redaction, and a signed envelope (canonical serialization plus Ed25519 sign/verify with fail-closed keyring trust and expected-subject binding, ADR-0007) now covers public lineage integrity at the library level, while encrypted private bundles, remote sinks, audited reads, retention, material-merge enforcement, and CLI/gate wiring of signature verification remain explicit follow-up controls rather than assumed protections.
...
- The version-2 public contract validates subject, execution, reviewer, allowlisted metadata, and redaction evidence. Default validation is v2-only; an explicit strict reader retains version 1 for historical readability, but legacy manifests do not satisfy the current allowlist policy. Neither format fetches GitHub reviewer state. `signPublicProvenance`/`verifySignedPublicProvenance` wrap version-2 manifests in a strict Ed25519 envelope (ADR-0007) whose verification fails closed on tampering, key substitution, and downgrade; the signature authenticates the signer's claimed lineage, not GitHub state (`src/provenance.ts`).
...
| Signed envelope JSON | `verifySignedPublicProvenance` | Local file/library -> verifier | Untrusted envelope input must fail closed on tampering, key substitution, replay, and downgrade | `src/provenance.ts`, `signedPublicProvenanceSchema` |
...
| TM-003 | PR author or compromised caller | Reviewer data is supplied by caller | Claim an approval that GitHub never authenticated | Material changes appear independently reviewed | Reviewer lineage, public manifest | Reviewer state is typed; signed envelopes require at least one approved reviewer and authenticate the signer's claimed lineage (`src/provenance.ts`, ADR-0007) | No GitHub review retrieval or head-SHA binding; signatures authenticate signer claims, not GitHub state | Resolve reviews through least-privilege GitHub API; bind reviewer, state, submitted SHA, and author separation | Compare manifest lineage with GitHub API during verification | High | High | High |
| TM-004 | Artifact or local-file attacker | Consumer accepts unsigned public CLI output | Modify subject, execution, or lineage after generation | Forged provenance accepted | Public manifest | Separate AI workflow signs a commit-scoped blob (`.github/workflows/ai-attestation-reusable.yml`); canonical serialization plus Ed25519 signed envelope with fail-closed keyring verification and expected-subject binding (`src/provenance.ts`, ADR-0007) | CLI/gate consumption of signed envelopes and keyring distribution are not wired; the cosign attestation remains a separate format | Canonical serialization; sign public manifest; verify exact repository, workflow, ref, and commit expectations | Record verification failures and signer identity | Medium | High | High |
...
|---|---|---|
| `src/provenance.ts` | Defines the public schema, allowlist, redaction, validation evidence, and the signed-envelope sign/verify contract | TM-001, TM-002, TM-003, TM-004 |
