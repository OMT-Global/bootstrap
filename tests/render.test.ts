...
describe("renderManagedFiles", () => {
  ...
  for (const kind of archetypes) {
    it(`renders a stable managed file set for ${kind}`, () => {
      ...
      expect(prWorkflow?.contents).toContain("Require Dependabot commits or verified main merges");
      ...
      expect(prWorkflow?.contents).toContain('(.parents | length) == 2');
      expect(prWorkflow?.contents).toContain('PR_BASE_SHA: ${{ github.event.pull_request.base.sha }}');
      expect(prWorkflow?.contents).toContain('PR_BASE_REPO: ${{ github.event.pull_request.base.repo.full_name }}');
      expect(prWorkflow?.contents).toContain('rev-list --first-parent "$PR_BASE_SHA"');
      expect(prWorkflow?.contents).toContain('[[ "$first_parent" == "$previous_commit" ]]');
      expect(prWorkflow?.contents).toContain('test("^Merge branch.*main.*into dependabot/")');
